...
def build_trees(json_file, outdir=None, consensus=False, db="data/gsaid.db",
                nboot=100, threads=1, maxsize=1000, cutoff=0.5, callback=None):
    """
    Main function for clustering script.  Generate bootstrap samples
    for feature vectors and calculate distance matrices for NJ reconstruction.

    :param json_file:  str, path to JSON file generated by minimap2.py
    :param outdir:  str, directory to write output files
    :param consensus:  bool, if True then generate consensus trees
    ...
    :param maxsize:  int, size of lineage above which disk-based processing
                     is used - otherwise calculations done in memory
    :param cutoff:  float, bootstrap threshold for consensus tree
    ...
    callback('loading lineage classifications from database')
    lineages = dump_lineages(db)

    callback('loading JSON')
    features = import_json(json_file)

    by_lineage = split_by_lineage(features, lineages)
    for lineage, lfeatures in by_lineage.items():
        n = len(lfeatures)
        callback('start {}, {} entries'.format(lineage, n))
        if n <= 3:
            callback('skipping small lineage (n<4)')
            continue

        # compute symmetric differences
        sym_diffs, m, labels = get_sym_diffs(lfeatures, use_file=True)
        callback('computed symmetric differences, '
                 'reduced down to {} variants'.format(len(labels)))

        # export labels
        outfile = open(os.path.join(outdir, '{}.labels.csv'.format(lineage)), 'w')
        ...
        if consensus:
            contree = majority_consensus(trees, cutoff=cutoff)
            callback('built consensus tree')
            Phylo.write(
                contree,
                file=os.path.join(outdir, '{}.cons.nwk'.format(lineage)),
                format='newick'
            )
        else:
            Phylo.write(
                trees,
                file=os.path.join(outdir, '{}.boot.nwk'.format(lineage)),
                format='newick'
            )
...
if __name__ == "__main__":
    ...
    args = parse_args()
    cb = Callback()
    build_trees(json_file=args.json, outdir=args.out, consensus=args.cons,
                db=args.db, nboot=args.nboot, threads=args.threads,
                cutoff=args.cutoff, callback=cb.callback)
