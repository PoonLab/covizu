...
def get_parents(input_tree):
    """ generate dictionary of child->parent associations """
    parents = {}
    for clade in input_tree.find_clades(order='level'):
        for child in clade:
...
def print_phylo(input_tree):
    """ DEBUGGING - format() not implemented for Clade objects """
    with StringIO() as handle:
        Phylo.write(input_tree, handle, "newick")
        print(handle.getvalue())


def annotate_tree(input_tree, tree_label_dict, minlen=0.5, callback=None):
    """
    ...
    tip_labels = set(tip.name for tip in input_tree.get_terminals())
    set_diff = tip_labels.difference(set(tree_label_dict.keys()))
    if len(set_diff) > 0 and callback:
        callback(
            "mismatch detected between tree and label file:",
            level='ERROR')
        callback(f"tip_labels: {tip_labels}\n", level='ERROR')
        callback(
            f"label_dict.keys(): {tree_label_dict.keys()}\n",
            level='ERROR')
        callback(f"set_diff: {set_diff}\n", level='ERROR')

    input_tree = collapse_polytomies(input_tree, minlen=minlen)  # label internal nodes
    ...
    for tip in input_tree.get_terminals():
        if '|' in tip.name:
            ...
            tip.labels = []
            for tip_label in tip.name.split('|'):
                tip.labels.extend(tree_label_dict[tip_label])
        else:
            tip.labels = tree_label_dict[tip.name]
    ...
    for node in input_tree.get_nonterminals():
        ...
        if node.name is None:
            ...
            for idx in node.name.split('|'):
                node.labels.extend(tree_label_dict[idx])
...
def serialize_tree(tree):
    ...
    for node in tree.find_clades(order='level'):
        ...
        # if not node.labels:
        #     # Set first date to first date of parent; last date to the earliest
        #     # first date of a descendant
        #     id = node
        ...
        obj['edges'].append([variant_d[parent], variant, round(
            node.branch_length, 2), node.confidence])
    #     date_diff = datetime.strptime(last_date, "%Y-%m-%d") - datetime.strptime(first_date,
    #                                                                           "%Y-%m-%d")

    #     # date width, first date, parent ID, child ID, branch length, node support
    #     if(variant_d[parent] in map_nodes):
    #         map_nodes[variant_d[parent]].append([date_diff, first_date, variant_d[parent],
    #                                               variant, round(node.branch_length, 2),
    #                          node.confidence])
    #     else:
    #         map_nodes[variant_d[parent]] = [[date_diff, first_date, variant_d[parent],
    #                                            variant, round(node.branch_length, 2),
    #                          node.confidence]]
...
def parse_args():
    ...
    parser.add_argument("tree", type=argparse.FileType(
        'r'), help="input, path to file with consensus tree or bootstrap trees")
    parser.add_argument("labels", type=argparse.FileType(
        'r'), help="input, path to file with sequence label to tip index map")
    parser.add_argument(
        "-o",
        "--outfile",
...
if __name__ == "__main__":
    ...
    if args.boot:
        ...
        try:
            ...
            print(f"Detected multiple trees in file, handling as bootstrap, exit {e}")
            trees = Phylo.parse(args.tree, 'newick')
            ctree = consensus(trees, cutoff=args.cutoff)

    # sequence labels keyed by integers mapping to tips
    out_label_dict = parse_labels(args.labels)
    out_tree = annotate_tree(ctree, out_label_dict, minlen=args.minlen)
    out_obj = serialize_tree(out_tree)
    args.outfile.write(json.dumps(out_obj, indent=2))
