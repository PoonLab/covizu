...
def call_pangolin(fasta_file, ncores=6, chunk_size=1000):
    """
    Call Pangolin with MPI - assumes script is run from project root
    :param fasta_file:  str, path to xz-compressed FASTA file
    :param ncores:  int, number of cores to run with MPI
    :param chunk_size:  int, number of records to read from FASTA at a time
    :return:  dict, lineage assignments keyed by sequence name
    """
    mpirun = ["mpirun", "--np", str(ncores)]
    tmp = tempfile.NamedTemporaryFile()
    cmd = ["python3", "covizu/utils/pangolin.py", '--quiet', fasta_file, tmp.name,
           '--size', str(chunk_size)]
    subprocess.check_call(mpirun+cmd)

    # collect output files generated by MPI threads
    results = {}
    for i in range(ncores):
        csvfile = open("{}.{}.csv".format(tmp.name, i))
        reader = DictReader(csvfile)
        for row in reader:
            results.update({row['taxon']: row['lineage']})
    return results
...
def parse_args():
    ...
    parser.add_argument("--vspango", type=str, help="input, path to VirusSeq gzip metadata TSV")
    ...
    parser.add_argument("--pangolin_cores", type=int, default=6,
                        help="option, number of cores to run Pangolin")
...
if __name__ == '__main__':
    ...
    if args.vspango is None:
        ...
        lineages = call_pangolin(args.vsfasta, ncores=args.pangolin_cores)
    else:
        lineages = {}
